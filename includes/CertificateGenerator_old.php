<?php
/**
 * INVENTIJN CERTIFICATE GENERATOR v2.1 - SYNTAX FIXED EDITION
 * Premium PDF Certificate Generator - FIXED syntax errors
 * Now with 100% working template and better error handling
 * 
 * Key Changes v2.1:
 * - Fixed syntax error around line 363 (quote escaping)
 * - Improved string concatenation for large templates
 * - Better error handling and validation
 * - Enhanced mPDF v8 compatibility
 * - Safer template generation
 * 
...
class CertificateGenerator {
    ...
    // Inventijn Brand Colors v2.1 - Consistent palette
    ...
    private function detectMPDFVersion() {
        if (class_exists('Mpdf\\Mpdf')) {
            ...
            error_log("✅ CertificateGenerator v2.1 using modern mPDF v8+");
        } elseif (class_exists('mPDF')) {
            $this->mpdf_version = 'mPDF';
            error_log("⚠️ CertificateGenerator v2.1 using legacy mPDF v5.7");
        } else {
    ...
    private function logSystemInfo() {
        error_log("🎯 CertificateGenerator v2.1 initialized:");
    ...
    public function generateCertificate($course_participant_id, $template = 'default', $certificate_type = 'deelname') {
        ...
        try {
            ...
            // Generate certificate HTML - NEW v2.1 template
    ...
    /**
     * Enhanced mPDF initialization with v2.1 optimizations
     */
    ...
    private function setUniversalMPDFSettings() {
        ...
        $this->mpdf->SetCreator('Inventijn Certificate System v2.1');
    ...
    /**
     * NEW v2.1 - Generate mPDF-compatible certificate HTML with FIXED syntax
     */
    private function generateCertificateHTML($template, $certificate_type) {
        ...
        // Get the NEW v2.1 template with FIXED syntax
    ...
    /**
     * NEW v2.1 - mPDF Compatible Template with FIXED SYNTAX
     * This template is specifically designed to work with mPDF limitations
     * - Fixed quote escaping issues that caused parse errors
     * - Better string concatenation for maintainability
     * - No flexbox/grid, uses tables for layout
     * - No external fonts, uses DejaVu Sans
     * - Simplified CSS that mPDF can handle
     */
...
<head>
  ...
  <title>Certificaat - {{participant_name}}</title>';
    ...
    private function getTemplateStyles() {
        return '  <style>
    /* v2.1 - Final Inventijn Template - Fixed Syntax */
  ...
  <div class="page-wrapper">
    <div class="certificate">
      ...
      <div class="header">
        <img src="/assets/images/logo.svg" class="logo" alt="Inventijn Logo">
    ...
    // Continue with all the existing methods from the original class...
    // (These are not changed, just included for completeness)
    ...
    private function hasExistingCertificate($course_participant_id, $certificate_type) {
        try {
            ...
            return false; // Assume no existing certificate on error
        }
    }
    ...
    private function validateSystemRequirements() {
        ...
        if ($memory_limit && $memory_limit !== '-1') {
            ...
            if ($memory_bytes < 128 * 1024 * 1024) { // 128MB minimum
                error_log("⚠️ Low memory limit detected: " . $memory_limit . " (recommend 128MB+)");
            }
        }
    }
    ...
    /**
     * Build enhanced email HTML with v2.1 styling
     */
        ...
        <body>
            <div class='container'>
                ...
                <div class='footer'>
                    ...
                    <p>Certificate System v2.1 | <a href='https://inventijn.nl'>inventijn.nl</a></p>
                </div>
            </div>
        </body>
        </html>";
    ...
    private function sendEnhancedEmail($to, $subject, $html_content, $attachment_path, $attachment_name) {
        ...
        $headers = [
            ...
            'X-Mailer: Inventijn Certificate System v2.1',
            'X-Priority: 1'
        ];
